...
{ /* FLIPBOOK.PageWebGL */
    FLIPBOOK.PageWebGL = function(book, i, matf, matb, hard, options, preloaderMatF, preloaderMatB) {
        ...
        this.cube = new THREE.Mesh(this.gF, new THREE.MeshFaceMaterial(mats))
...
this.receiveShadow  = true*/
        ...
        this.cubeEmpty = new THREE.Mesh(
            new THREE.BoxGeometry(this.pW, this.pH, 0.01, 1, 1, 0),
            new THREE.MeshFaceMaterial(mats2)
        )
        this.cubeEmpty.position.x = this.pW * 0.5;


        /*this.cubeSmall = new THREE.Mesh(this.gF.clone(), new THREE.MeshFaceMaterial(basicMat2))
        this.cubeSmall.position.x = this.pW * 0.5;
        this.add(this.cubeSmall)*/
        ...
        this.b2.constraint = MOD3.ModConstant.RIGHT;
        if (this.pH > this.pW)
            this.b2.switchAxes = true;
        this.b2.force = Math.PI / 2
        this.b2.offset = .1
        ...
        var self = this
        jQuery(options.main).bind('pageLoaded', function(e, i, size) {

            //console.log('loaded page '+i+' size '+size)
            var index = self.options.rightToLeft ? self.book.pages.length - self.index - 1 : self.index;
            var p1 = self.options.rightToLeft ? 2 * index + 1 : 2 * index
            var p2 = self.options.rightToLeft ? 2 * index : 2 * index + 1

            if (i == p1) {

                if (self.frontTextureSize >= size)
                    return
                self.frontTextureSize = size

                if (!self.frontTextures[size]) {
                    var c1 = self.options.pages[p1].canvas[size]
                    var t1 = new THREE.CanvasTexture(c1)

                    t1.anisotropy = self.book.renderer.getMaxAnisotropy();
                        //t1.generateMipmaps = false
                    t1.needsUpdate = true
                    t1.repeat.x = c1.scaleX
                    t1.repeat.y = c1.scaleY
                    t1.repeat.y = c1.scaleY
                    t1.offset.y = 1-c1.scaleY
                    self.frontTextures[size] = t1
                    self.frontMaterial = self.createMaterial(t1, 'back')
                    self.frontMaterial.needsUpdate = true
                    self.setFrontMat(self.frontMaterial)
                    delete c1
                } else {
                    self.frontMaterial.map = self.frontTextures[size]
                    self.frontMaterial.needsUpdate = true
                }

                self.book.updateHtmlLayer()

            } else if (i == p2) {

                if (self.backTextureSize >= size)
                    return
                self.backTextureSize = size


                if (!self.backTextures[size]) {
                    var c2 = self.options.pages[p2].canvas[size]
                    var t2 = new THREE.CanvasTexture(c2)
                        //t2.generateMipmaps = false

                        t2.anisotropy = self.book.renderer.getMaxAnisotropy();
                    t2.needsUpdate = true
                    t2.repeat.x = c2.scaleX
                    t2.repeat.y = c2.scaleY
                    t2.repeat.y = c2.scaleY
                    t2.offset.y = 1-c2.scaleY
                    self.backTextures[size] = t2
                    self.backMaterial = self.createMaterial(t2, 'back')
                    self.backMaterial.needsUpdate = true
                    self.setBackMat(self.backMaterial)
                    delete c2
                } else {
                    self.backMaterial.map = self.backTextures[size]
                    self.backMaterial.needsUpdate = true
                }
    ...
    FLIPBOOK.PageWebGL.prototype.load = function(side, size, callback) {
        //size = 2048
        size = size || 128
            //size = 512
        ...
        /*if((side == 'front' && this.loadedF[size]) || (side == 'back' && this.loadedB[size]) || (this.loadedF[size] && this.loadedB[size]))
            return*/
        ...
        if (options.pdfMode) {
            ...
            main.loadPagesFromPdf(arr, size, function() {
                callback.call(self)
            })
        } else {
            ...
            if(self.frontMaterial && self.backMaterial){
                self.setFrontMat(self.frontMaterial)
                self.setBackMat( self.backMaterial)
                callback.call(self)
                return
            ...
            var ldr = new THREE.TextureLoader();
                ...
                self.options.main.setLoadingProgress(.3)
    ...
    FLIPBOOK.PageWebGL.prototype.setFrontMat = function(m) {
        ...
        if (this.cube.material.materials[4] === m)
            return
        this.cube.material.materials[4] = m;
    ...
    FLIPBOOK.PageWebGL.prototype.setBackMat = function(m) {
        ...
        if (this.cube.material.materials[5] === m)
            return
        this.cube.material.materials[5] = m;
        //this.backMaterial = m
    ...
    FLIPBOOK.PageWebGL.prototype.disposeMat = function() {
        ...
        var matF = this.cube.material.materials[4]
        var matB = this.cube.material.materials[5]
...
{ /* FLIPBOOK.BookWebGL */
    ...
    FLIPBOOK.BookWebGL.prototype.init3d = function() {
        ...
        this.options.hasHtmlContent = true
        ...
        if (this.options.lights) {
            ...
            sl.intensity = sIntensity;
            sl.position.set(1000, 0, 2000);
            sl.distance = 4000
            sl.penumbra = .1;
            sl.decay = 0;
            /*sl.castShadow = true
...
*/
    ...
    FLIPBOOK.BookWebGL.prototype.initHtmlContent = function() {
        ...
        htmlLayer.className = "htmlLayer"
        var $htmlLayer = jQuery(htmlLayer)
        this.$htmlLayer = $htmlLayer
            //jQuery(number).append(jQuery('<img src="images/book1/page1.jpg">'))

        this.$pageR = jQuery('<div>')
            .css('width', 1000 * this.options.pageWidth / this.options.pageHeight + 'px')
            .css('height', '1000px')
        ...
        this.$pageL = jQuery('<div>')
            .css('width', 1000 * this.options.pageWidth / this.options.pageHeight + 'px')
            .css('height', '1000px')
            .css('position', 'absolute')
            .css('top', '-500px')
            .css('left', -1000 * this.options.pageWidth / this.options.pageHeight + 'px')
            //.css('background', 'rgba(255,0,0,.2)')
            .css('pointer-events', 'none')
            .appendTo($htmlLayer)

        this.$pageLInner = jQuery('<div>')
            .css('pointer-events', 'all')
            .appendTo(this.$pageL)

        this.htmlLayer = new THREE.CSS3DObject(htmlLayer);
    ...
    FLIPBOOK.BookWebGL.prototype.onResize = function() {
        ...
        var zoomMin = Number(this.options.zoomLevels[0])
        ...
        if (r1 > 2 * r2 * s ) {
            ...
            if (this.options.responsiveView) {
                ...
                this.view = 2
                if (this.rightIndex == 0) this.focusRighttPage();
                else this.focusBothPages()

            } else {
                
                if (this.flippedleft == 0)
                    this.focusRighttPage();
            ...
            if (this.options.responsiveView && w <= this.options.responsiveViewTreshold) {
                //switch to single page mode
                this.view = 1

                if (this.rightIndex == 0 || this.isFocusedRight()) this.focusRighttPage();
                else this.focusLeftPage();
    ...
    FLIPBOOK.BookWebGL.prototype.createPages = function() {
        ...
        for (i = 0; i < p.length / 2; i++) {
            ...
            page = new FLIPBOOK.PageWebGL(self, i, texturefront, textureback, hardness, self.options, preloaderMatF, preloaderMatB);
    ...
    FLIPBOOK.BookWebGL.prototype.nextPage = function(load) {
       ...
       /* if(beforePrevPage)
        ...
        if (this.view == 1) {
            ...
            if (this.flippedleft == 0)
                this.focusBothPages(500)
            else if (this.flippedright == 1)
                this.focusLeftPage(500)
        }
    ...
    FLIPBOOK.BookWebGL.prototype.turnPageComplete = function() {
        ...
        var L = this.options.pageTextureSize
        var S = this.options.pageTextureSize
        ...
        if (leftPage) {
            leftPage.load("back", L, function() {

                if (rightPage)
                    rightPage.load("front", L, function() {
                        main.setLoadingProgress(1)
                        leftPage.load("front", S, function() {})
                        rightPage.load("back", S, function() {})
                        if (prevPage) {
                            prevPage.load("back", S, function() {})
                        }
                        if (nextPage) {
                            nextPage.load("front", S, function() {})
                        }
                    });
                else {
                    main.setLoadingProgress(1)
    ...
    FLIPBOOK.BookWebGL.prototype.focusLeftPage = function(time, delay) {
    ...
    FLIPBOOK.BookWebGL.prototype.focusRighttPage = function(time, delay) {
    ...
    FLIPBOOK.BookWebGL.prototype.focusBothPages = function(time, delay) {
    ...
    FLIPBOOK.BookWebGL.prototype.moveToPos = function(pos, time, delay) {
        ...
        if (time && this.movingTo != pos && this.centerContainer.position.x != pos) {
            ...
            var self = this
            this.movingTo = pos
            var tween = new TWEEN.Tween(this.centerContainer.position).to({
                    x: pos
                }, time)
                .easing(TWEEN.Easing.Sinusoidal.Out)
                .onUpdate(function() {
                ...
                .onComplete(function() {
                    ...
                    if (self.htmlLayer) {
                        self.htmlLayer.position.x = self.centerContainer.position.x
                        self.htmlLayer.position.y = self.centerContainer.position.y
                    }
    ...
    FLIPBOOK.BookWebGL.prototype.updateHtmlLayer = function() {
        ...
        if (this.rightIndex == 0) {
            ...
            if (this.options.pages[this.rightIndex - 1].htmlContent) {

                this.$pageLInner.empty();
                var content = jQuery(this.options.pages[this.rightIndex - 1].htmlContent).appendTo(this.$pageLInner);
                this.$pageL.show();
        ...
        if (this.rightIndex == this.pages.length * 2) {
            ...
            if (this.options.pages[this.rightIndex].htmlContent) {

                this.$pageRInner.empty()
                var content = jQuery(this.options.pages[this.rightIndex].htmlContent).appendTo(this.$pageRInner)
                this.$pageR.show()
                this.htmlLayerVisible = true
    ...
    FLIPBOOK.BookWebGL.prototype.zoomTo = function(amount, time) {
        ...
        if (newZoom == this.options.zoom) {
            ...
            this.centerContainer.position.set(0, 0, 0);
            if (this.view == 1) {
                focusedLeft ? this.focusLeftPage() : this.focusRighttPage();
            } else {
